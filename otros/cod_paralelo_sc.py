...
def main():
    if rank == 0:
        ...
        while z <= size:
            ...
            t_com_ini = time()
            listb = dict(numero=z, t_com_inicial=t_com_ini, comunas=comunas, rango_1=r_1, rango_2=r_2)
            comm.send(listb, dest=z)
    if rank != 0:
        t_recibido = time()
        lista = comm.recv(source=0)
        t_comunicacion = lista["t_com_inicial"] - t_recibido
        contarVoto(lista["comunas"], lista["rango_1"], lista["rango_2"], rank, name, t_comunicacion, t_recibido)
    if rank == 0:
        izquerda = 0
        derecha = 0
        independiente = 0
